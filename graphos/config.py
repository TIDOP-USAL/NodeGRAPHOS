...
# parse arguments
processopts = ['dataset', 'featextract', 'featmatch', 'gcps', 'ori', 'dense', 
               'export_point_cloud', 'mesh', 'dem', 'ortho', 'odm_report', 'odm_postprocess']

rerun_stages = {
    'add_images': 'dataset',
    'featextract': 'features',
    'featmatch': 'matches',
    'gcps': 'gcp',
    'ori': 'ori',
    'dense': 'mvs',  # O el directorio donde almacenas densificación
    'export_point_cloud': 'pointcloud',
    'mesh': 'mesh',
    'dem': 'dem',
    'ortho': 'ortho',
...
def config(argv=None, parser=None):
    ...
    parser.add_argument('--max-features-number',
                        metavar='<positive integer>',
                        action=StoreValue,
                        default=5000,
                        type=int,
                        help='Maximum number of features to detect. Default: %(default)s')

    parser.add_argument('--octave-resolution',
                        metavar='<positive integer>',
                        action=StoreValue,
                        default=3,
                        type=int,
                        help='SIFT: Number of layers in each octave. Default: %(default)s')

    parser.add_argument('--contrast-threshold',
                        metavar='<float>',
                        action=StoreValue,
                        default=0.006667,
                        type=float,
                        help='SIFT: Contrast Threshold. Default: %(default)s')

    parser.add_argument('--edge-threshold',
                        metavar='<float>',
                        action=StoreValue,
                        default=10.0,
                        type=float,
                        help='SIFT: Threshold used to filter out edge-like features. Default: %(default)s')
    ...
    parser.add_argument('--ratio',
                        ...
                        default=0.8,
                        type=float,
                        help='Matching ratio. Default: %(default)s')
    ...
    parser.add_argument('--distance',
                        ...
                        default=0.7,
                        type=float,
                        help='Matching distance. Default: %(default)s')
    ...
    parser.add_argument('--max-error',
                        ...
                        default=4.0,
                        type=float,
                        help='Matching maximum error. Default: %(default)s')
    ...
    parser.add_argument('--confidence',
                        ...
                        default=0.999,
                        type=float,
                        help='Matching confidence. Default: %(default)s')

    parser.add_argument('--cross-check',
                        action=StoreTrue,
                        nargs=0,
                        default=True,
                        help='Enable cross checking. Default: true')

    parser.add_argument('--exhaustive-matching',
                        action=StoreTrue,
                        nargs=0,
                        default=False,
                        help='Force exhaustive matching (disables spatial matching if georef is present). Default: false')

    # Orientation Options

    parser.add_argument('--fix-calibration',
                        action=StoreTrue,
                        nargs=0,
                        default=False,
                        help='Fix camera calibration. Default: False')

    parser.add_argument('--use-rtk-accuracy',
                        action=StoreTrue,
                        nargs=0,
                        default=False,
                        help='Use RTK positioning accuracy. Default: False')

    parser.add_argument('--use-gcp',
                        action=StoreTrue,
                        nargs=0,
                        default=True,
                        help='Use Ground Control Points for absolute orientation. Default: True')

    parser.add_argument('--use-poses',
                        action=StoreTrue,
                        nargs=0,
                        default=True,
                        help='Use camera poses for absolute orientation. Default: True')
    ...
    parser.add_argument('--camera-lens',
                        ...
                        action=StoreValue,
                        default='OpenCV 1',
                        choices=['Pinhole 1', 'Pinhole 2', 'Radial 1', 'Radial 2', 'OpenCV 1', 'OpenCV Fisheye', 'OpenCV 2', 'Radial Fisheye 1', 'Radial Fisheye 2', 'Radial 3'],
                        help=('Set a camera projection type. Manually setting a value '
                            'can help improve geometric undistortion. Can be one of: %(choices)s. Default: '
                            '%(default)s'))

    # Dense Reconstruction Options

    parser.add_argument('--resolution-level',
                        metavar='<positive integer>',
                        type=int, 
                        default=2, 
                        action=StoreValue,
                        help='MVS: Resolution Level. Default: %(default)s')
    ...
    parser.add_argument('--min-resolution',
                        ...
                        default=640, 
                        action=StoreValue,
                        help='MVS: Minimum image resolution. Default: %(default)s')

    parser.add_argument('--max-resolution',
                        metavar='<positive integer>',
                        type=int, 
                        default=3200,
                        action=StoreValue,
                        help='MVS: Maximum image resolution. Default: %(default)s')

    parser.add_argument('--number-views',
                        metavar='<positive integer>',
                        type=int, 
                        default=5, 
                        action=StoreValue,
                        help='MVS: Number of views to match per image. Default: %(default)s')

    parser.add_argument('--number-views-fuse',
                        metavar='<positive integer>',
                        type=int, 
                        default=3, 
                        action=StoreValue,
                        help='MVS: Number of views used for fusion. Default: %(default)s')

    parser.add_argument('--estimate-colors',
                        action=StoreTrue, 
                        nargs=0,
                        default=True,
                        help='MVS: Estimate colors for dense point cloud. Default: %(default)s')

    parser.add_argument('--estimate-normals',
                        action=StoreTrue, 
                        nargs=0,
                        default=True,
                        help='MVS: Estimate normals for dense point cloud. Default: %(default)s')

    parser.add_argument('--segment',
                        action=StoreTrue, 
                        nargs=0,
                        default=False,
                        help='Enable automatic segmentation for dense point cloud. Default: %(default)s')
    ...
    parser.add_argument('--depth',
                        ...
                        default=11,
                        action=StoreValue,
                        help='Maximum reconstruction depth for Poisson. Controls the detail level of the mesh. Default: %(default)s')

    parser.add_argument('--boundary-type',
                        metavar='<string>',
                        choices=['Free', 'Dirichlet', 'Neumann'],
                        default='Free',
                        action=StoreValue,
                        help='Boundary type for Poisson reconstruction. Default: %(default)s')

    # Export point cloud Options

    parser.add_argument('--save-colors', 
                        action=StoreTrue, 
                        nargs=0,
                        default=True,
                        help='Export colors for point cloud. Default: %(default)s')
    ...
    parser.add_argument('--dem-resolution',
                        ...
                        type=float,
                        default=0.1,
                        help='DSM/DTM resolution in m / pixel. Note that this value is capped by a ground sampling distance (GSD) estimate.'
                             ' Default: %(default)s')
    ...
    parser.add_argument('--orthophoto-resolution',
                        ...
                        default=0.05,
                        type=float,
                        help=('Orthophoto resolution in m / pixel. Note that this value is capped by a ground sampling distance (GSD) estimate.'
                              'Default: %(default)s'))
    ...
    parser.add_argument('--interpolation',
                        ...
                        default='BILINEAR',
                        action=StoreValue,
                        help='Interpolation type for orthophoto. Default: %(default)s')
