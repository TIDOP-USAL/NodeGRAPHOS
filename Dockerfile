...
#Da problemas con ODM. Si utilizo la versión de ODM me faltan modulos
...
#    make && \
#    make install && \
#    cd /app && rm -rf /tmp/laszip

# Descargar LASzip desde la misma URL que ODM
...
RUN git clone --branch 290 --depth 1 https://github.com/OpenDroneMap/entwine /tmp/entwine && \
    ...
    cmake .. -GNinja \
          -DCMAKE_INSTALL_PREFIX=${INSTALL_PREFIX} \
	      -DWITH_TESTS=OFF \
          # Parece que ignora esta opción
          -DWITH_ZSTD=OFF \ 
	      -DCMAKE_BUILD_TYPE=Release && \
    ninja && \
    ninja install && \
    cd /app && rm -rf /tmp/entwine	
